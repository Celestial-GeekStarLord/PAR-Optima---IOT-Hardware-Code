...
const int rotateTime = 450;    // Adjust for ~90 or 60 degrees
const int forwardLittle = 300; // Time to move forward slightly after rotation
...
void loop() {
  ...
  if (Firebase.ready() && (millis() - lastFirebaseCheck > checkInterval)) {
    ...
    if (Firebase.RTDB.getString(&fbdo, "/cmd/data")) {
      ...
      if (newCmd == "w" && stopCount == 0) {
        ...
        isPaused = false;
        currentCmd = "w"; // Resume normal line follow logic
      } 
  ...
  // Follow line only if not paused
...
// First Stop: Entered 'a' -> Rotate Left -> Move Forward slightly
void handleFirstStopAction() {
  Serial.println("Executing Action A: Rotate Left");
  turnLeft();
  delay(rotateTime);
  moveForward();
  delay(forwardLittle);
...
// Second Stop: Entered 's' -> Rotate Right -> Move Forward slightly
void handleSecondStopAction() {
  Serial.println("Executing Action S: Rotate Right");
  turnRight();
  delay(rotateTime);
  moveForward();
  delay(forwardLittle);
...
void followLine() {
  ...
  // Condition: Robot is lost (All sensors see White/High)
    ...
    Serial.println("Line Lost! Stopping and updating Firebase...");
    stopRobot();
    isPaused = true;
    stopCount++; // Increment stop counter
    
    // Update Firebase to 'x' to notify user/app robot is waiting
    if (Firebase.ready()) {
      Firebase.RTDB.setString(&fbdo, "/cmd/data", "x");
    }
  }
